...
export default {
  ...
  methods: {
        ...
        roll: function (amplifier, minAmount) {
            var randomNumber = Math.max(Math.floor(Math.random() * amplifier) + 1, minAmount);
            return randomNumber;
        },
        heal: function () {
        ...
        roundStatus: function (type, userMessage, monsterMessage) {
            if (String(type).toLowerCase() == 'heal') {
                this.messages.push({
                    message: 'You healed for ' + userMessage + ' hitpoints this round'
                }, {
                        message: 'Monster did ' + monsterMessage + ' damage this round'
                    });
            } else {
                this.messages.push({
